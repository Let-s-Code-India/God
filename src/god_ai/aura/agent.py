...
def agent(goal: str, root: Optional[Path] = None, max_iterations: int = 3, allow_edits: bool = False) -> AgentResult:
    ...
    for iteration in range(1, max_iterations + 1):
        try:
            completed = subprocess.run(["python", "-m", "pytest", "-q"], cwd=workspace, capture_output=True, text=True, timeout=300)
        ...
        if allow_edits:
            _apply_unified_diff(advice, workspace)
        else:
            return AgentResult(goal, False, iteration, last_output + "\nSuggested fix:\n" + advice)
    return AgentResult(goal, False, max_iterations, last_output)


def _apply_unified_diff(text: str, workspace: Path) -> None:
    ...
    try:
        subprocess.run(["patch", "-p1", "--forward", "--batch", "-i", str(patch_file)], cwd=workspace, check=False, capture_output=True, text=True, timeout=60)
    finally:
