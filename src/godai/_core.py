"""Small shared primitives used by the four public godai libraries."""
...
LOGGER = logging.getLogger("godai")
...
class Settings:
    ...
    provider: str = os.getenv("GODAI_PROVIDER", "offline")
    timeout: float = float(os.getenv("GODAI_TIMEOUT", "20"))
    retries: int = int(os.getenv("GODAI_RETRIES", "2"))
    circuit_threshold: int = int(os.getenv("GODAI_CIRCUIT_THRESHOLD", "3"))
    cache_ttl: float = float(os.getenv("GODAI_CACHE_TTL", "3600"))
...
def llm(prompt: str, *, provider: Optional[str] = None, timeout: Optional[float] = None) -> str:
    ...
    if not os.getenv(key_names.get(chosen, "GODAI_API_KEY")):
        raise ProviderUnavailable(f"No credentials configured for {chosen}. Set {key_names.get(chosen, 'GODAI_API_KEY')} to use this provider.")
    from godai.aether import circuit_breaker, timeout as bounded_timeout
...
def _provider_request(provider: str, prompt: str, timeout: float) -> str:
    ...
    import requests
    base = os.getenv("GODAI_BASE_URL", "")
    if provider == "ollama":
        base = base or os.getenv("OLLAMA_BASE_URL", "http://localhost:11434")
        response = requests.post(base.rstrip("/") + "/api/generate", json={"model": os.getenv("GODAI_MODEL", "llama3"), "prompt": prompt, "stream": False}, timeout=timeout)
        response.raise_for_status()
        return str(response.json().get("response", ""))
    if provider == "anthropic":
        response = requests.post(os.getenv("GODAI_BASE_URL", "https://api.anthropic.com/v1/messages"), json={"model": os.getenv("GODAI_MODEL", "claude-3-haiku-20240307"), "max_tokens": 1024, "messages": [{"role": "user", "content": prompt}]}, headers={"x-api-key": os.getenv("ANTHROPIC_API_KEY", ""), "anthropic-version": "2023-06-01"}, timeout=timeout)
        response.raise_for_status(); return str(response.json()["content"][0]["text"])
    if provider == "gemini":
        endpoint = os.getenv("GODAI_BASE_URL", "https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent")
    ...
    if provider in {"openai", "groq", "xai", "qwen", "openrouter", "openai_compatible"}:
        endpoint = base or os.getenv("GODAI_BASE_URL", "https://api.openai.com/v1")
        response = requests.post(endpoint.rstrip("/") + "/chat/completions", json={"model": os.getenv("GODAI_MODEL", "default"), "messages": [{"role": "user", "content": prompt}]}, headers={"Authorization": f"Bearer {os.getenv('GODAI_API_KEY', '')}"}, timeout=timeout)
        response.raise_for_status()
